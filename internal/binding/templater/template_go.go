...
import (
	...
	"github.com/fizzywhizbang/qt/internal/binding/converter"
...
func GoTemplate(module string, stub bool, mode int, pkg, target, tags string) []byte {
	...
	if !utils.QT_GEN_GO_WRAPPER() && module == "AndroidExtras" {
		utils.Save(utils.GoQtPkgPath(strings.ToLower(module), "utils-androidextras_android.go"), utils.Load(filepath.Join(strings.TrimSpace(utils.GoListOptional("{{.Dir}}", "github.com/fizzywhizbang/qt/internal", "-find", "get files dir")), "/binding/files/utils-androidextras_android.go")))
	} else if module == "Qml" {
		cont := utils.Load(filepath.Join(strings.TrimSpace(utils.GoListOptional("{{.Dir}}", "github.com/fizzywhizbang/qt/internal", "-find", "get files dir")), "/binding/files/utils-qml.go"))
...
`,
	...
	for _, m := range append(parser.GetLibs(), "qt", "strings", "unsafe", "log", "runtime", "fmt", "errors", "js", "time", "hex", "reflect", "math", "sync", "strconv", "internal", "gow") {
		...
		if strings.Contains(inputString, fmt.Sprintf(" %v.", mlow)) ||
			...
			case "qt":
				fmt.Fprintln(bb, "\"github.com/fizzywhizbang/qt\"")

			case "internal":
				fmt.Fprintln(bb, "\"github.com/fizzywhizbang/qt/internal\"")

			case "gow":
				fmt.Fprintln(bb, "\"github.com/fizzywhizbang/qt/interop/gow\"")
			...
			default:
				if mode == MOC {
					fmt.Fprintf(bb, "std_%[1]v \"github.com/fizzywhizbang/qt/%[1]v\"\n", mlow)
				} else {
					fmt.Fprintf(bb, "\"github.com/fizzywhizbang/qt/%v\"\n", mlow)
				}
		...
		if module == "gui" && utils.QT_API_NUM(utils.QT_VERSION()) >= 5050 && !utils.QT_GEN_GO_WRAPPER() {
			if mode == NONE {
				fmt.Fprintln(bb, "_ \"github.com/fizzywhizbang/qt/internal/binding/runtime\"")
