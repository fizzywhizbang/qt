...
func GetImports(path, target, tagsCustom string, level int, onlyDirect bool) []string {
	...
	if onlyDirect {
		...
		defer func() {
			...
			if importsQmlOrQuick == "" {
				...
				stdImport[len(stdImport)-1] = "github.com/fizzywhizbang/qt/internal/binding/runtime"

				cmd := utils.GoList(fmt.Sprintf("{{if not .Standard}}{{if eq .ImportPath \"%v\"}}{{else}}{{range .Imports}}{{if eq . \"github.com/fizzywhizbang/qt/qml\" \"github.com/fizzywhizbang/qt/quick\"}}{{.}}{{end}}{{end}}{{end}}{{end}}", strings.Join(stdImport, "\" \"")), "-deps", utils.BuildTags(tags))
			...
			if importsInterop == "" {
				...
				cmd := utils.GoList(fmt.Sprintf("{{if not .Standard}}{{if eq .ImportPath \"%v\"}}{{else}}{{range .Imports}}{{if eq . \"github.com/fizzywhizbang/qt/interop\"}}{{.}}{{end}}{{end}}{{end}}{{end}}", strings.Join(stdImport, "\" \"")), "-deps", utils.BuildTags(tags))
				if !utils.UseGOMOD(path) || (utils.UseGOMOD(path) && !strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/")) {
					cmd.Dir = path
				} else if utils.UseGOMOD(path) && strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/") {
					cmd.Dir = filepath.Dir(utils.GOMOD(path))
			...
			if importsFlutter == "" {
				...
				cmd := utils.GoList(fmt.Sprintf("{{if not .Standard}}{{if eq .ImportPath \"%v\"}}{{else}}{{range .Imports}}{{if eq . \"github.com/fizzywhizbang/qt/flutter\"}}{{.}}{{end}}{{end}}{{end}}{{end}}", strings.Join(stdImport, "\" \"")), "-deps", utils.BuildTags(tags))
				if !utils.UseGOMOD(path) || (utils.UseGOMOD(path) && !strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/")) {
					cmd.Dir = path
				} else if utils.UseGOMOD(path) && strings.Contains(strings.Replace(path, "\\", "/", -1), "/vendor/") {
					cmd.Dir = filepath.Dir(utils.GOMOD(path))
	...
	for _, l := range libs {
		...
		go func(l string) {
			...
			if strings.Contains(l, "github.com/fizzywhizbang/qt") && !(strings.Contains(l, "qt/internal") || strings.Contains(l, "qt/flutter") || strings.Contains(l, "qt/interop")) {
				if strings.Contains(l, "github.com/fizzywhizbang/qt/") {
					importedStdMutex.Lock()
...
func GetQtStdImports() (o []string) {
	importedStdMutex.Lock()
	for k := range importedStd {
		o = append(o, strings.TrimPrefix(k, "github.com/fizzywhizbang/qt/"))
	}
	importedStdMutex.Unlock()
	return
}
...
func QtModVersion(path string) string {
	cmd := exec.Command("go", "list", "-m", "-f", "{{.Version}}", "github.com/fizzywhizbang/qt")
	cmd.Dir = path
...
func RestartWithPinnedVersion(path string) bool {
	...
	cmd = exec.Command("go", "install", "-v", "-tags=no_env", "github.com/fizzywhizbang/qt/cmd/...")
	cmd.Dir = path
