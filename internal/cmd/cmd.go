...
func ParseFlags() bool {
	...
	if api := utils.QT_API(""); api != "" && err == nil && !utils.QT_DOCKER() && !forDocker {
		if utils.GoListOptional("{{.Dir}}", "github.com/fizzywhizbang/qt/internal/binding/files/docs/"+api, "-find", "get doc dir") == "" {
			...
			if !utils.UseGOMOD("") {
				if o := utils.GoListOptional("{{join .Imports \"|\"}}", "github.com/fizzywhizbang/qt/internal/binding/files/docs", "get doc dir"); o != "" {
					for _, v := range strings.Split(o, "|") {
						fmt.Println(strings.TrimPrefix(strings.TrimSpace(strings.Replace(v, "'", "", -1)), "github.com/fizzywhizbang/qt/internal/binding/files/docs/"))
					}
				}
			} else {
				...
				for mid := 5; mid <= 15; mid++ {
					for min := 0; min <= 5; min++ {
						...
						go func(mid, min int) {
							...
							if utils.GoListOptional("{{.Dir}}", "github.com/fizzywhizbang/qt/internal/binding/files/docs/"+v, "-find", "get doc dir") != "" {
								fmt.Println(v)
							}
							wg.Done()
						}(mid, min)
...
func InitEnv(target string, docker bool, path string) {
	...
	if !utils.ExistsDir(utils.QT_DIR()) {
		qt_dir := strings.TrimSpace(utils.RunCmd(utils.GoList("{{.Dir}}", "github.com/fizzywhizbang/env_"+runtime.GOOS+"_amd64_"+strconv.Itoa(utils.QT_VERSION_NUM())[:3], "-find"), "get env dir"))
		...
		case "windows":
			...
			if err == nil {
				...
				utils.RemoveAll(link)
				_, err = utils.RunCmdOptionalError(exec.Command("cmd", "/C", "mklink", "/J", link, strings.TrimSpace(utils.RunCmd(utils.GoList("{{.Dir}}", "github.com/fizzywhizbang/env_"+runtime.GOOS+"_amd64_"+strconv.Itoa(utils.QT_VERSION_NUM())[:3]+"/Tools", "-find"), "get env dir"))), "create symlink for env")
			}
		}
...
func virtual(arg []string, target, path string, writeCacheToHost bool, docker bool, system string) {
	...
	if utils.UseGOMOD(path) {
		...
		if writeCacheToHost {
			...
			if strings.Contains(path, "github.com/fizzywhizbang/qt/internal/examples") && !strings.Contains(path, "github.com/fizzywhizbang/qt/internal/examples/androidextras") {
				gpath += pathseperator + gpfs
			} else {
