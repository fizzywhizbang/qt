...
func Generate(target string, docker, vagrant bool) {
	...
	utils.Save(filepath.Join(runtimePath, "utils-runtime.go"), utils.Load(filepath.Join(strings.TrimSpace(utils.GoListOptional("{{.Dir}}", "github.com/fizzywhizbang/qt/internal", "-find", "get files dir")), "/binding/files/utils-runtime.go")))
